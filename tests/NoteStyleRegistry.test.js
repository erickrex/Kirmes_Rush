...
// Sample note style data based on actual Rythm Foundation format (rythm.json)
const sampleRythmStyle = {
  version: '1.1.0',
  name: "Rythm'",
  ...
  assets: {
    ...
    judgementSick: {
      assetPath: 'default:ui/popup/rythm/sick',
      scale: 0.65,
      isPixel: false
    },
    judgementGood: {
      assetPath: 'default:ui/popup/rythm/good',
      scale: 0.65
    },
    comboNumber0: {
      assetPath: 'default:ui/popup/rythm/num0',
      scale: 0.45
    },
    countdownTwo: {
      assetPath: 'shared:ui/countdown/rythm/ready',
      scale: 1.0,
      data: {
        audioPath: 'shared:gameplay/countdown/rythm/introTWO'
      }
    }
  }
};
...
describe('NoteStyleRegistry', () => {
  ...
  describe('parseEntryDataRaw', () => {
    it('should parse valid note style data', () => {
      const result = registry.parseEntryDataRaw(sampleRythmStyle, 'rythm.json');

      expect(result).not.toBeNull();
      expect(result.name).toBe("Rythm'");
    ...
    it('should parse pixel style with fallback', () => {
      ...
      expect(result.fallback).toBe('rythm');
    });
  ...
  describe('createEntry', () => {
    it('should create entry from style data', () => {
      const data = registry.parseEntryDataRaw(sampleRythmStyle, 'rythm.json');
      const entry = registry.createEntry('rythm', data);

      expect(entry).not.toBeNull();
      expect(entry.id).toBe('rythm');
      expect(entry.name).toBe("Rythm'");
  ...
  describe('Style Access Methods', () => {
    ...
    beforeEach(() => {
      ...
      testRegistry.setMockData('rythm', sampleRythmStyle);
      testRegistry.setMockData('pixel', samplePixelStyle);
      testRegistry.loadEntries(['rythm', 'pixel']);
    });

    it('should get style data', () => {
      const data = testRegistry.getStyleData('rythm');
      expect(data).not.toBeNull();
      expect(data.name).toBe("Rythm'");
    });
    ...
    it('should get style name', () => {
      expect(testRegistry.getStyleName('rythm')).toBe("Rythm'");
      expect(testRegistry.getStyleName('nonexistent')).toBe('nonexistent');
    });

    it('should get fallback', () => {
      expect(testRegistry.getFallback('rythm')).toBeNull();
      expect(testRegistry.getFallback('pixel')).toBe('rythm');
    });

    it('should get asset', () => {
      const noteAsset = testRegistry.getAsset('rythm', 'note');
      expect(noteAsset).not.toBeNull();
      expect(noteAsset.assetPath).toBe('shared:notes');
    });

    it('should return null for non-existent asset', () => {
      expect(testRegistry.getAsset('rythm', 'nonexistent')).toBeNull();
    });

    it('should fall back to fallback style for missing assets', () => {
      // pixel style doesn't have judgementSick, but rythm does
      const judgement = testRegistry.getAsset('pixel', 'judgementSick');
      expect(judgement).not.toBeNull();
      expect(judgement.assetPath).toBe('default:ui/popup/rythm/sick');
    });

    it('should check if style has asset', () => {
      expect(testRegistry.hasAsset('rythm', 'note')).toBe(true);
      expect(testRegistry.hasAsset('rythm', 'nonexistent')).toBe(false);
    });
  });
  ...
  describe('Note Data Methods', () => {
    ...
    it('should check if splashes are enabled', () => {
      expect(testRegistry.areSplashesEnabled('rythm')).toBe(true);
    });

    it('should get judgement asset', () => {
      const sickAsset = testRegistry.getJudgementAsset('rythm', 'sick');
      expect(sickAsset).not.toBeNull();
      expect(sickAsset.scale).toBe(0.65);
    });

    it('should get combo number asset', () => {
      const num0Asset = testRegistry.getComboNumberAsset('rythm', 0);
      expect(num0Asset).not.toBeNull();
      expect(num0Asset.scale).toBe(0.45);
    });

    it('should get countdown asset', () => {
      const twoAsset = testRegistry.getCountdownAsset('rythm', 'Two');
      expect(twoAsset).not.toBeNull();
      expect(twoAsset.data.audioPath).toBe('shared:gameplay/countdown/rythm/introTWO');
    });
  });
  ...
  describe('Path Resolution', () => {
    ...
    it('should resolve default: prefix', () => {
      const resolved = registry.resolveAssetPath('default:ui/popup/rythm/sick');
      expect(resolved).toBe('preload/images/ui/popup/rythm/sick');
    });
  ...
  describe('Listing Methods', () => {
    ...
    it('should get style display info', () => {
      const info = testRegistry.getStyleDisplayInfo('rythm');

      expect(info).not.toBeNull();
      expect(info.id).toBe('rythm');
      expect(info.name).toBe("Rythm'");
  ...
  describe('Asset Path Collection', () => {
    ...
    it('should get all asset paths for a style', () => {
      const paths = testRegistry.getAllAssetPaths('rythm');
    ...
    it('should include fallback paths', () => {
      ...
      // Should also include rythm's paths via fallback
      expect(paths).toContain('preload/images/ui/popup/rythm/sick');
    });
  });
