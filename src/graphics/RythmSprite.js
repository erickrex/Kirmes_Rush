/**
 * @fileoverview RythmSprite - Extended Phaser Sprite with Rythm Foundation-specific functionality
 * Provides animation helpers, offset handling, and z-index support.
 *
 * Ported from source/rythm/graphics/RythmSprite.hx and source/rythm/play/stage/Bopper.hx
 */
...
/**
 * @typedef {Object} RythmSpriteConfig
...
/**
 * Extended Phaser Sprite with Rythm Foundation-specific functionality.
 * Features:
 ...
 */
class RythmSprite extends Phaser.GameObjects.Sprite {
  /**
  ...
  /**
   * Create a new RythmSprite with a static texture.
  ...
  static create(scene, x = 0, y = 0, key) {
    const sprite = new RythmSprite(scene, x, y, key);
    scene.add.existing(sprite);
    return sprite;
  }

  /**
   * Create a new RythmSprite configured for pixel art.
   * @param {Phaser.Scene} scene - The scene
   * @param {number} x - X position
   * @param {number} y - Y position
   * @param {string} key - Texture key
  ...
  playAnimation(animName, restart = false, ignoreOther = false) {
    ...
    if (!animationKey) {
      console.warn(`[RythmSprite] Animation not found: ${correctedName}`);
      return this;
    }
    ...
    if (!this.anims || !this.anims.animationManager.exists(animationKey)) {
      if (this.anims.exists(animationKey)) {
        ...
        console.warn(`[RythmSprite] Animation not found: ${animationKey}`);
        return this;
      }
    } else {
  ...
  _correctAnimationName(name, fallback = 'idle') {
    ...
    if (lastDash !== -1) {
      ...
      console.warn(`[RythmSprite] Animation "${name}" not found, trying "${stripped}"`);
      return this._correctAnimationName(stripped, fallback);
    }

    // Try fallback
    if (fallback && fallback !== name) {
      console.warn(`[RythmSprite] Animation "${name}" not found, falling back to "${fallback}"`);
      return this._correctAnimationName(fallback, undefined);
    }

    console.error(`[RythmSprite] Animation "${name}" not found!`);
    return null;
  }
   ...
   */
  clone() {
    const cloned = new RythmSprite(this.scene, this.x, this.y, this.texture.key, this.frame.name);
    ...
    // Copy RythmSprite-specific properties
