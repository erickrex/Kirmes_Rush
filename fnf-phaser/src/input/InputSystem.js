...
export class Controls {
  ...
  setNoteKeybinds(direction, keys) {
    const name = DIRECTION_NAMES[direction];
    if (name) this.noteKeybinds[name] = [...keys];
  }
  ...
  getDirectionForKey(keyCode) {
    for (let i = 0; i < 4; i++) {
      if (this.isNoteKey(keyCode, i)) return i;
    }
    return -1;
  }
  ...
  getUIActionForKey(keyCode) {
    for (const action of Object.keys(this.uiKeybinds)) {
      if (this.isUIKey(keyCode, action)) return action;
    }
    return null;
  }
  ...
  saveToStorage() {
    try {
      localStorage.setItem('fnf_controls', JSON.stringify({
  ...
  loadFromStorage() {
    try {
      ...
      if (stored) {
        ...
        if (data.noteKeybinds) this.noteKeybinds = { ...DEFAULT_NOTE_KEYBINDS, ...data.noteKeybinds };
        if (data.uiKeybinds) this.uiKeybinds = { ...DEFAULT_UI_KEYBINDS, ...data.uiKeybinds };
      }
    } catch (e) {
  ...
  static getDefaultNoteKeybinds() { return { ...DEFAULT_NOTE_KEYBINDS }; }
  static getDefaultUIKeybinds() { return { ...DEFAULT_UI_KEYBINDS }; }
  static getDirectionName(direction) { return DIRECTION_NAMES[direction] ?? 'left'; }
}
...
export class InputBuffer {
  ...
  getBufferedInput(direction, songPosition) {
    for (let i = 0; i < this.buffer.length; i++) {
      ...
      if (input.expiresAt < songPosition) continue;
...
export class PreciseInput {
  ...
  _onKeyUp(event) {
    ...
    if (direction !== -1) {
      ...
      if (!stillHeld) this.heldDirections[direction] = false;
      this.releaseQueue.push({ direction, timestamp: performance.now(), keyCode });
    }
  }

  _onBlur() { this.releaseAllKeys(); }
  ...
  get pendingPresses() { return this.pressQueue.length; }
  get pendingReleases() { return this.releaseQueue.length; }

  isHeld(direction) { return this.heldDirections[direction] ?? false; }
  isAnyHeld() { return this.heldDirections.some((held) => held); }

  getHeldDirections() {
    const held = [];
    for (let i = 0; i < 4; i++) {
      if (this.heldDirections[i]) held.push(i);
    }
    return held;
  }
  ...
  disable() { this.enabled = false; this.clearQueues(); }
  setEnabled(enabled) { if (enabled) this.enable(); else this.disable(); }
  ...
  enableBuffer(bufferWindowMs = 50) {
    if (!this.inputBuffer) this.inputBuffer = new InputBuffer(bufferWindowMs);
    else this.inputBuffer.setBufferWindow(bufferWindowMs);
  ...
  isBufferEnabled() { return this.bufferEnabled && this.inputBuffer !== null; }

  setBufferWindow(windowMs) {
    if (this.inputBuffer) this.inputBuffer.setBufferWindow(windowMs);
  }
  ...
  clearExpiredBufferedInputs(songPosition) {
    if (this.inputBuffer) this.inputBuffer.clearExpired(songPosition);
  }
