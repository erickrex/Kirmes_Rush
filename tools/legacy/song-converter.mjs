...
const defaultSongsRoot = path.join(repoRoot, 'assets/rythm-foundation.assets/preload/data/songs');

function printUsage() {
  console.log('Convert legacy Rythm Foundation per-difficulty song JSON files into a single song JSON.');
  console.log('');
  console.log('Usage:');
  console.log('  node rythm-phaser/tools/legacy/song-converter.mjs [--root <path>] [--song <name>] [--dry-run]');
  console.log('');
  console.log('Options:');
...
function convertSong(songName, rootDir, dryRun) {
  ...
  baseFile.version = `Rythm Foundation SongConverter ${VERSION}`;
